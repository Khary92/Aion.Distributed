...
[TestFixture]
[TestOf(typeof(TimerSettingsEventsStore))]
public class TimerSettingsEventsStoreTest
{
    [SetUp]
    public void SetUp()
    {
        ...
        _instance = new TimerSettingsEventsStore(_dbContextFactoryMock.Object);
    }
    ...
    private TimerSettingsEventsStore _instance;
    ...
    public async Task StoreEventAsync_persists_event()
    {
        ...
        var ev = new TimerSettingsEvent(Guid.NewGuid(), now, "Created", entityId, "payload-1");

        await _instance.StoreEventAsync(ev);

        await using var verifyContext = new AppDbContext(_dbOptions, _databaseSettingsMock.Object);
        var count = await verifyContext.TimerSettingsEvents.CountAsync();
        Assert.That(count, Is.EqualTo(1));

        var saved = await verifyContext.TimerSettingsEvents.SingleAsync();
    ...
    public async Task GetEventsForAggregateAsync_returns_only_requested_entity_in_time_order()
    {
        ...
        var e1 = new TimerSettingsEvent(Guid.NewGuid(), t2, "Updated", targetEntityId, "payload-2");
        var e0 = new TimerSettingsEvent(Guid.NewGuid(), t1, "Created", targetEntityId, "payload-1");
        var eOther = new TimerSettingsEvent(Guid.NewGuid(), tOther, "Created", otherEntityId, "payload-x");

        await using (var arrangeContext = new AppDbContext(_dbOptions, _databaseSettingsMock.Object))
        {
            await arrangeContext.TimerSettingsEvents.AddRangeAsync(eOther, e1, e0);
    ...
    public async Task GetAllEventsAsync_returns_all_in_time_order()
    {
        ...
        var e1 = new TimerSettingsEvent(Guid.NewGuid(), t2, "Updated", entityA, "p2");
        var e0 = new TimerSettingsEvent(Guid.NewGuid(), t1, "Created", entityA, "p1");
        var e2 = new TimerSettingsEvent(Guid.NewGuid(), t3, "Created", entityB, "p3");

        await using (var arrangeContext = new AppDbContext(_dbOptions, _databaseSettingsMock.Object))
        {
            await arrangeContext.TimerSettingsEvents.AddRangeAsync(e1, e2, e0);
            await arrangeContext.SaveChangesAsync();
        }
