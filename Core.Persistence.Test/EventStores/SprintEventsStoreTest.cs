...
[TestFixture]
[TestOf(typeof(SprintEventsStore))]
public class SprintEventsStoreTest
{
    [SetUp]
    public void SetUp()
    {
        ...
        _instance = new SprintEventsStore(_dbContextFactoryMock.Object);
    }

    private Mock<IDbContextFactory<AppDbContext>> _dbContextFactoryMock;
    private DbContextOptions<AppDbContext> _dbOptions;
    private SprintEventsStore _instance;
    ...
    public async Task StoreEventAsync_persists_event()
    {
        ...
        var ev = new SprintEvent(Guid.NewGuid(), now, "Created", entityId, "payload-1");

        await _instance.StoreEventAsync(ev);

        await using var verifyContext = new AppDbContext(_dbOptions);
        var count = await verifyContext.SprintEvents.CountAsync();
        Assert.That(count, Is.EqualTo(1));

        var saved = await verifyContext.SprintEvents.SingleAsync();
    ...
    public async Task GetEventsForAggregateAsync_returns_only_requested_entity_in_time_order()
    {
        ...
        var e1 = new SprintEvent(Guid.NewGuid(), t2, "Updated", targetEntityId, "payload-2");
        var e0 = new SprintEvent(Guid.NewGuid(), t1, "Created", targetEntityId, "payload-1");
        var eOther = new SprintEvent(Guid.NewGuid(), tOther, "Created", otherEntityId, "payload-x");

        await using (var arrangeContext = new AppDbContext(_dbOptions))
        {
            await arrangeContext.SprintEvents.AddRangeAsync(eOther, e1, e0);
    ...
    public async Task GetAllEventsAsync_returns_all_in_time_order()
    {
        ...
        var e1 = new SprintEvent(Guid.NewGuid(), t2, "Updated", entityA, "p2");
        var e0 = new SprintEvent(Guid.NewGuid(), t1, "Created", entityA, "p1");
        var e2 = new SprintEvent(Guid.NewGuid(), t3, "Created", entityB, "p3");

        await using (var arrangeContext = new AppDbContext(_dbOptions))
        {
            await arrangeContext.SprintEvents.AddRangeAsync(e1, e2, e0);
            await arrangeContext.SaveChangesAsync();
        }
