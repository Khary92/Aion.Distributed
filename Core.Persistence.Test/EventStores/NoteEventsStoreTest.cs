...
public class NoteEventsStoreTest
{
    ...
    {
        var noteEvent =
            new NoteEvent(Guid.NewGuid(), DateTimeOffset.Now, "EventType", Guid.NewGuid(), "EventPayload");
    ...
    public async Task StoreEventAsync_persists_event()
    {
        ...
        var noteEvent = new NoteEvent(Guid.NewGuid(), now, "Created", entityId, "payload-1");
        ...
        var saved = await verifyContext.NoteEvents.SingleAsync();
    ...
    public async Task GetEventsForAggregateAsync_returns_only_requested_entity_in_time_order()
    {
        ...
        var e1 = new NoteEvent(Guid.NewGuid(), t2, "Updated", targetEntityId, "payload-2");
    ...
    public async Task GetAllEventsAsync_returns_all_in_time_order()
    {
        ...
        var e1 = new NoteEvent(Guid.NewGuid(), t2, "Updated", entityA, "p2");
