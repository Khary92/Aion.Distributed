...
public class StatisticsDataEventsStoreTest
{
    ...
    public async Task StoreEventAsync_persists_event()
    {
        ...
        var ev = new StatisticsDataEvent(Guid.NewGuid(), now, "Created", entityId, "payload-1");

        await _instance.StoreEventAsync(ev);

        await using var verifyContext = new AppDbContext(_dbOptions, _databaseSettingsMock.Object);
        var count = await verifyContext.StatisticsDataEvents.CountAsync();
        Assert.That(count, Is.EqualTo(1));

        var saved = await verifyContext.StatisticsDataEvents.SingleAsync();
    ...
    public async Task GetEventsForAggregateAsync_returns_only_requested_entity_in_time_order()
    {
        ...
        var e1 = new StatisticsDataEvent(Guid.NewGuid(), t2, "Updated", targetEntityId, "payload-2");
        var e0 = new StatisticsDataEvent(Guid.NewGuid(), t1, "Created", targetEntityId, "payload-1");
        var eOther = new StatisticsDataEvent(Guid.NewGuid(), tOther, "Created", otherEntityId, "payload-x");

        await using (var arrangeContext = new AppDbContext(_dbOptions, _databaseSettingsMock.Object))
        {
            await arrangeContext.StatisticsDataEvents.AddRangeAsync(eOther, e1, e0);
    ...
    public async Task GetAllEventsAsync_returns_all_in_time_order()
    {
        ...
        var e1 = new StatisticsDataEvent(Guid.NewGuid(), t2, "Updated", entityA, "p2");
        var e0 = new StatisticsDataEvent(Guid.NewGuid(), t1, "Created", entityA, "p1");
        var e2 = new StatisticsDataEvent(Guid.NewGuid(), t3, "Created", entityB, "p3");

        await using (var arrangeContext = new AppDbContext(_dbOptions, _databaseSettingsMock.Object))
        {
            await arrangeContext.StatisticsDataEvents.AddRangeAsync(e1, e2, e0);
            await arrangeContext.SaveChangesAsync();
        }
