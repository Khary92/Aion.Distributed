...
public class AnalysisByTicketDecorator(AnalysisByTicket analysisByTicket, ITagRequestsService tagRequestsService)
{
    ...
    public async Task<Dictionary<Productivity, List<Guid>>> GetMostRepresentedTagIdsByProductivity()
    {
        ...
        var usedTags = await tagRequestsService.GetTagsByTagIds(tagIds);
        ...
        analysisByTicket.StatisticData
            ...
            .ForEach(x =>
            {
                ...
                if (x.statisticData.IsUnproductive)
                {
                    countByTagUnproductive[x.tagId]++;
                }
            });
